...
MODELS: list[tuple[str, list[Path], list[Path], str, str]] = [
    ...
    ("DPRNN", [_paths.THIRD_PARTY / "Dual-Path-RNN-Pytorch"],
     [_paths.WEIGHTS / "dprnn_musdb"], "run",
     "官方只发语音权重 → 已用本机 GPU **自训** MUSDB18 4-stem 权重（step 31900；SI-SDR 仍为负，仅作负面证据）"),
...
MODULES = [
    ("01_models", _paths.MODELS,
     [("separation", _paths.SEP_MODELS), ("denoising", _paths.DEN_MODELS),
      ("_third_party", _paths.THIRD_PARTY), ("_weights", _paths.WEIGHTS),
      ("_selfimpl", _paths.SELF_IMPL),
      ("MODEL_REGISTRY.md", _paths.MODELS / "MODEL_REGISTRY.md"),
      ("<板块>/<模型>/code (抽样)", _paths.model_code_dir("BS-RoFormer-L12"))]),
